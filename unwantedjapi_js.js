...
export const adminAPI = {
  // MongoDB endpoints for admin dashboard
  getUsers: () => api.get('/mongo/admin/users'),
  deleteUser: (mongoId) => {
    if (!mongoId) {
      return Promise.reject(new Error('MongoDB User ID is required'));
    }
    // Convert mongoId if it's an object with _id property
    const validId = typeof mongoId === 'string' ? mongoId : getMongoId(mongoId);
    
    if (!validId || !isValidMongoId(validId)) {
      console.error('Invalid MongoDB ID format:', mongoId);
      return Promise.reject(new Error('Invalid MongoDB User ID format. Expected 24 hex characters.'));
    }
    console.log('🗑️ Admin API - Delete User:', validId);
    return api.delete(`/mongo/admin/users/${validId}`);
  },
  getOrders: () => api.get('/mongo/admin/orders'),
